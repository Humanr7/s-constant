...
model_root = os.path.join(models_path, 'Stable-cascade')
...
def predict(prompt, negative_prompt, width, height, guidance_scale, prior_steps, decoder_steps, seed, batch_size):
    device = "cuda"
    prior = StableCascadePriorPipeline.from_pretrained("stabilityai/stable-cascade-prior", torch_dtype=torch.bfloat16, cache_dir=f"{models_path}/Stable-cascade/.cache/prior").to(device)
    ...
    decoder = StableCascadeDecoderPipeline.from_pretrained("stabilityai/stable-cascade",  torch_dtype=torch.float16, cache_dir=f"{models_path}/Stable-cascade/.cache/decoder").to(device)
...
def on_ui_tabs():
    with gr.Blocks() as stable_cascade_block:
        with ResizeHandleRow():
            with gr.Column():
                ...
                ctrls = [prompt, negative_prompt, width, height, guidence_scale, prior_steps, decoder_steps, sampling_seed, batch_size]
    ...
    return [(stable_cascade_block, "Cascade", "stable_cascade")]
